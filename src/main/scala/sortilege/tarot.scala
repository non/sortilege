...
object Tarot {
  ...
  object Draw {
    def apply(card: Card)(implicit gen: Generator): Draw =
      if (gen.nextBoolean) Upright(card) else Inverted(card)
  }
...
"""
  ...
  def shuffle(implicit gen: Generator): Vector[Card] =
    Deck.qshuffled

  def single(implicit gen: Generator): Draw =
    Draw(Deck.qchoose)

  def draw(n: Int)(implicit gen: Generator): Vector[Draw] =
    shuffle.take(n).map(Draw(_))

  def threeCard(implicit gen: Generator): ThreeCard = {
    val Vector(a, b, c) = draw(3)
    ThreeCard(a, b, c)
  }

  def horseshoe(implicit gen: Generator): Horseshoe = {
    val Vector(a, b, c, d, e) = draw(5)
    Horseshoe(a, b, c, d, e)
  }

  def celticCross(implicit gen: Generator): CelticCross = {
    val Vector(a, b, c, d, e, f, g, h, i, j) = draw(10)
    CelticCross(Circle(a, b, c, d, e, f), Staff(g, h, i, j))
  }
}
